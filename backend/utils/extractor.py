...
def extract_letters(image_name):
    data_dir = r"../backend/prediction/"
    path_img = os.path.join(data_dir, "images", image_name)

    dist_path = os.path.join(data_dir, "letters")
            ...
            if cv2.contourArea(cnt) > 0:
                for i in range(w+8):
                    if cv2.pointPolygonTest(cnt, (x+w+8-i, y+h+50), False) > 0:
                        [x1, y1, w1, h1] = union(cv2.boundingRect(cnt), [x, y, w, h])
                        cv2.rectangle(im_copy, (x1, y1), (x1+w1, y1+h1), (0, 0, 0), 2)
                        return [x1, y1, w1, h1]
        return False
            ...
            if detection(x, y, w, h) != True:
                if h < 16:
                    if fusion_i(x, y, w, h) != False:
                        contours, _ = cv2.findContours(setTresh(im_copy), cv2.RETR_TREE, cv2.CHAIN_APPROX_NONE)
